...
export class TabComponent implements OnInit {
  ...
  constructor(
    ...
    @Inject(MAT_DIALOG_DATA) public data: { clickedRow: any, tab: any, parent: any },
  ...
  ngOnInit(): void {
    this.cazzeonService.request(`api/tab?tabId=${this.data.tab.id}`, HttpMethod.GET, async (response: Response) => {
      ...
      this.constructFields(this.data.tab.fields);
      this.tabReady = true;
  ...
  /**
   * This function will construct the grid and form fields of the current tab.
   * 
  ...
  constructFields(fields: any[]): void {
    ...
    this.tabData.allFields = newFields;
  }

  /**
   * Construct the items of the context menu. This is made in the tab component because the buttons and process are tab related.
   * 
   * @param items Items from response. This items needs to be converted into ContextMenuItem interface.
   */
  constructMenuItems(tabData: any): void {
    const tab = this;
    this.tabData.contextMenuItems = [
      {
        label: CONTEXT_MENU.actionsLabel, imageSource: CONTEXT_MENU.actionsIcon, items: this.constructProcessItems(tabData.buttonAndProcess)
      },
      {
        label: CONTEXT_MENU.tabsLabel, imageSource: CONTEXT_MENU.tabsIcon, items: this.constructTabItems(tabData.tabs)
      },
      {
        label: CONTEXT_MENU.deleteLabel, imageSource: CONTEXT_MENU.deleteIcon, clickFn(clickedRow, item) {
          tab.cazzeonService.request(`api/entity/delete/${tab.data.tab.entityName}`, HttpMethod.DELETE,
            (response: Response) => {
              tab.grid.rows = tab.grid.rows.filter(row => row !== clickedRow);
            },
            async (response: Response) => {
   ...
   * @returns Context menu item array representing the process buttons of the current tab.
   */
   ...
   * @returns Context menu item array representing the tabs of the current tab.
   */
  ...
  dropFilterColumn(event: CdkDragDrop<any[]>): void {
    if (event.previousIndex !== event.currentIndex) {
      ...
      this.grid.currentGridFieldsIndexedByHqlProperty = indexArrayByProperty(this.tabData.gridFields, HQL_PROPERTY);
